...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[0, 2].plot(df['s/g distance'], df['Execution time (ms)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
axs[0, 2].set_xlabel('s/g distance')
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 0].plot(df['s/g distance'], df['Memory Allocation (KB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 1].plot(df['s/g distance'], df['RSS (KB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 2].plot(df['s/g distance'], df['VMS (KB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
axs[1, 2].set_xlabel('s/g distance')
