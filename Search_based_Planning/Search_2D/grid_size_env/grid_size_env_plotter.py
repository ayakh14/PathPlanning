...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[0, 2].plot(df['Grid size'], df['Execution time (ms)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
axs[0, 2].set_xlabel('Grid size')
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 0].plot(df['Grid size'], df['Memory Allocation (KB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 1].plot(df['Grid size'], df['RSS (KB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 2].plot(df['Grid size'], df['VMS (KB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
axs[1, 2].set_xlabel('Grid size')
