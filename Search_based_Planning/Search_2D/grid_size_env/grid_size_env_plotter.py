...
for file_name, algo_name in data_files:
    temp_df = pd.read_csv(os.path.join('100_runs_results', file_name))
...
# Ensure Grid size is a float
df['Grid size'] = df['Grid size'].astype(float)
...
for metric in metrics:
    ...
    for i, algo in enumerate(algorithms):
        ...
        means = algo_df.groupby('Grid size')[metric].mean()
        std_devs = algo_df.groupby('Grid size')[metric].std()
    ...
    plt.title(f'{metric} vs Grid Size for Each Algorithm', fontsize=14)
    plt.xlabel('Grid Size', fontsize=12)
    plt.ylabel(metric, fontsize=12)
    plt.yscale('log')
    plt.legend()
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_Grid_Size_line_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
for metric in metrics:
    ...
    plt.ylabel(metric)
    plt.yscale('log')
    plt.title(f'{metric} vs Grid Size for Each Algorithm')
    # Add legend
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_Grid_Size_error_bar_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
for metric in metrics:
    ...
    for i, algo in enumerate(algorithms):
        algo_df = df[df['Algorithm'] == algo]
        algo_grid_sizes = sorted(algo_df['Grid size'].unique())

        if algo_grid_sizes != grid_sizes:
            missing_sizes = list(set(grid_sizes) - set(algo_grid_sizes))
            # print(f"Algorithm '{algo}' is missing data for grid sizes: {missing_sizes}")

        means = algo_df.groupby('Grid size')[metric].mean().reindex(grid_sizes, fill_value=0)
    ...
    plt.legend()
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_Grid_Size_bar_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
for metric in metrics:
    ...
    # Draw boxplots
    box_plot = sns.boxplot(data=df, x='Grid size', y=metric, hue='Algorithm', width=box_width, palette=colors)
...
for metric in metrics:
    for algo in algorithms:
        algo_df = df[df['Algorithm'] == algo]
        for sg_dist in algo_df['Grid size'].unique():
            sg_dist_df = algo_df[algo_df['Grid size'] == sg_dist]
...
table_df = pd.DataFrame(table_data, columns=['Algorithm', 'Metric', 'Grid size', 'Mean', 'Median', 'Standard Deviation', 'Minimum', 'Maximum'])
