...
for metric in metrics:
    ...
    plt.title(f'{metric} vs Walls size for Each Algorithm', fontsize=14)
    plt.xlabel('Grid number', fontsize=12)
    plt.ylabel(metric, fontsize=12)
    plt.yscale('log')
    plt.legend()
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_wall_size_line_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
for metric in metrics:
    ...
    plt.ylabel(metric)
    plt.yscale('log')
    plt.title(f'{metric} vs Walls size for Each Algorithm')
    # Add legend
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_wall_size_error_bar_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
#     plt.legend()
#     plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_wall_size_bar_plot_100_run.png"))  # Save the plot as png
#     plt.close()
