...
# Ensure Grid number is a float
df['Grid number'] = df['Grid number'].astype(float)
...
# Define directory to save the images
...
for metric in metrics:
    ...
    for i, algo in enumerate(algorithms):
        ...
        means = algo_df.groupby('Grid number')[metric].mean()
        std_devs = algo_df.groupby('Grid number')[metric].std()
    ...
    plt.title(f'{metric} vs Number of walls for Each Algorithm', fontsize=14)
    plt.xlabel('Grid number', fontsize=12)
    plt.ylabel(metric, fontsize=12)
    plt.yscale('log')
    plt.legend()
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_number_of_walls_line_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
for metric in metrics:
    ...
    plt.ylabel(metric)
    plt.yscale('log')
    plt.title(f'{metric} vs Number of walls for Each Algorithm')
    # Add legend
    ...
    plt.grid(True)
    plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_number_of_walls_error_bar_plot_100_run.png"))  # Save the plot as png
    plt.close()
...
# grid_sizes = sorted(df['Grid number'].unique())
...
#     for i, algo in enumerate(algorithms):
#         algo_df = df[df['Algorithm'] == algo]
#         algo_grid_sizes = sorted(algo_df['Grid number'].unique())

#         if algo_grid_sizes != grid_sizes:
#             missing_sizes = list(set(grid_sizes) - set(algo_grid_sizes))
#             # print(f"Algorithm '{algo}' is missing data for Grid numbers: {missing_sizes}")

#         means = algo_df.groupby('Grid number')[metric].mean().reindex(grid_sizes, fill_value=0)
...
#     plt.legend()
#     plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_vs_number_of_walls_bar_plot_100_run.png"))  # Save the plot as png
#     plt.close()
...
#     # Draw boxplots
#     box_plot = sns.boxplot(data=df, x='Grid number', y=metric, hue='Algorithm', width=box_width, palette=colors)
...
#     plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_Distribution_box_plot_100_run.png"))  # Save the plot as png
#     plt.close()
...
#     plt.legend()
#     plt.savefig(os.path.join(save_dir, f"{metric.replace(' ', '_')}_Boxplot_for_Each_Algorithm_100_run.png"))  # Save the plot as png
#     plt.close()
...
for metric in metrics:
    for algo in algorithms:
        algo_df = df[df['Algorithm'] == algo]
        for sg_dist in algo_df['Grid number'].unique():
            sg_dist_df = algo_df[algo_df['Grid number'] == sg_dist]
...
table_df = pd.DataFrame(table_data, columns=['Algorithm', 'Metric', 'Grid number', 'Mean', 'Median', 'Standard Deviation', 'Minimum', 'Maximum'])
