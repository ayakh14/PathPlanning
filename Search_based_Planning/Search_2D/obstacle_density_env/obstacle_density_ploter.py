...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 0].plot(df['Obstacle density'], df['Memory consumption (MB)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
...
for i, df in enumerate(dataframes):
    style = line_styles[i % len(line_styles)]  # Cycle through line styles
    marker = markers[i % len(markers)]  # Cycle through markers
    axs[1, 1].plot(df['Obstacle density'], df['Execution time (s)'], label=algorithm_names[i], linestyle=style, marker=marker, alpha=0.7)
